...
const btnBuscar = document.querySelectorAll('#searchsubmit');
  for (const iterator2 of btnBuscar){    
    iterator2.className += " btn btn-primary btn-block";
  }
...
const listWi = document.querySelector('.bluu_wi ul');
if(formularioBuscar){
    listWi.querySelectorAll('li').forEach(item => { item.className+='list-group-item' });
    listWi.className+= 'list-group';
}
